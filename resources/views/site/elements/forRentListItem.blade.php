@if($item) 
    <div class="forrent-post line-bottom">
        <div class="post-content"><a class="news-post-image-link" href="/for-rents/{{ $item->slug}}">
            @if ($item->img)
            ...
            @endif
            <h3 class="post-title title"><a class="news-post-link" href="/for-rents/{{ $item->slug}}">{{ $item->title }}</a></h3>
            <div class="row">
            ...
            <div class="col rent-attribute"><strong>Quận/huyện :</strong><span> {{ $item->district_id }}</span></div>
            </div>
